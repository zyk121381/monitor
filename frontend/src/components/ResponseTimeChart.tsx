...
const ResponseTimeChart: React.FC<ResponseTimeChartProps> = ({
  ...
  const { chartData, timeRange, yAxisMax } = useMemo(() => {
    ...
    let responseTimeData: DataPoint[] = processedHistory
      .map((item, idx) => {
        try {
          ...
          // 使用响应时间，或者默认值0
          const responseTime = (item && item.response_time !== undefined && item.response_time > 0) 
    ...
    // 设置y轴最大值
    const calculatedYAxisMax = maxResponseTime < 10000 ? Math.ceil(maxResponseTime * 1.1) : 10000;
