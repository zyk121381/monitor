...
export const getNotificationConfig = async () => {
  try {
    ...
    console.log(
      "通知配置格式化后:",
      JSON.stringify(formattedData).substring(0, 200) + "..."
    );
...
export const saveNotificationSettings = async (
  ...
  try {
    ...
    console.log(
      "monitors设置:",
      JSON.stringify(settings.monitors).substring(0, 200)
    );
    console.log(
      "agents设置:",
      JSON.stringify(settings.agents).substring(0, 200)
    );
    console.log(
      "特定监控数量:",
    ...
    const monitorSettings = {
      ...
      on_response_time_threshold: Boolean(
        settings.monitors.onResponseTimeThreshold
      ),
      response_time_threshold: Number(
        settings.monitors.responseTimeThreshold || 1000
      ),
      on_consecutive_failure: Boolean(settings.monitors.onConsecutiveFailure),
      consecutive_failure_threshold: Number(
        settings.monitors.consecutiveFailureThreshold || 3
      ),
    ...
    for (const monitorId in settings.specificMonitors) {
      ...
      try {
        ...
        const specificMonitorSettings = {
          ...
          on_response_time_threshold: Boolean(
            monitorSetting.onResponseTimeThreshold
          ),
          response_time_threshold: Number(
            monitorSetting.responseTimeThreshold || 1000
          ),
          on_consecutive_failure: Boolean(monitorSetting.onConsecutiveFailure),
          consecutive_failure_threshold: Number(
            monitorSetting.consecutiveFailureThreshold || 3
          ),
        ...
        console.log(
          `发送特定监控设置 ${monitorId}:`,
          JSON.stringify(specificMonitorSettings)
        );
        saveRequests.push(
          axios.post(
            API_ENDPOINTS.NOTIFICATION_SETTINGS,
            specificMonitorSettings
          )
        );
      } catch (err) {
    ...
    for (const agentId in settings.specificAgents) {
      ...
      try {
        ...
        console.log(
          `发送特定客户端设置 ${agentId}:`,
          JSON.stringify(specificAgentSettings)
        );
        saveRequests.push(
...
export const createNotificationChannel = async (channelData: any) => {
  try {
    const response = await axios.post(
      API_ENDPOINTS.NOTIFICATION_CHANNELS,
      channelData
    );
...
export const updateNotificationChannel = async (
  ...
  try {
    const response = await axios.put(
      API_ENDPOINTS.NOTIFICATION_CHANNEL_DETAIL(Number(channelId)),
      channelData
    );
    return response.data;
  } catch (error) {
