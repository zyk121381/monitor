  ...
  const fetchData = async () => {
    try {
      ...
      if (!agentResponse.success || !agentResponse.agent) {
        throw new Error(
          agentResponse.message || t("common.error.fetch", "获取数据失败")
        );
      }
      ...
      // 计算资源使用百分比（如果后端未计算）
      if (
        agentData.memory_total &&
        agentData.memory_used &&
        agentData.memory_usage === undefined
      ) {
        agentData.memory_usage =
          (agentData.memory_used / agentData.memory_total) * 100;
      }

      if (
        agentData.disk_total &&
        agentData.disk_used &&
        agentData.disk_usage === undefined
      ) {
        agentData.disk_usage =
          (agentData.disk_used / agentData.disk_total) * 100;
      }
      ...
      setError(
        error instanceof Error
          ? error.message
          : t("agents.fetchDetailFailed", "获取客户端详情失败")
      );
      Alert.alert(
        t("common.error", "错误"),
        t("agents.fetchDetailFailed", "获取客户端详情失败")
          ...
          onPress: async () => {
            try {
              ...
              if (result.success) {
                ...
                Alert.alert(
                  t("common.error", "错误"),
                  result.message || t("agents.deleteFailed", "删除客户端失败")
                );
              }
            } catch (error) {
      ...
      >
        ...
        <View style={styles.card}>
          <Text style={styles.cardTitle}>
            {t("agents.basicInfo", "基本信息")}
          </Text>
          <View style={styles.detailItem}>
            <Text style={styles.detailLabel}>
              {t("agents.hostname", "主机名")}:
            </Text>
            <Text style={styles.detailValue}>
              {agent.hostname || t("common.unknown", "未知")}
            </Text>
          </View>
          <View style={styles.detailItem}>
            <Text style={styles.detailLabel}>
              {t("agents.ipAddress", "IP地址")}:
            </Text>
            {agent.ip_addresses ? (
              <View>
                {(() => {
          ...
          <View style={styles.detailItem}>
            ...
            <Text style={styles.detailValue}>
              {agent.version || t("common.unknown", "未知")}
            </Text>
          </View>
          <View style={styles.detailItem}>
            <Text style={styles.detailLabel}>
              {t("agents.lastSeen", "最后在线")}:
            </Text>
            <Text style={styles.detailValue}>
              {timeSince(agent.last_seen || agent.updated_at)}
            </Text>
          </View>
          <View style={styles.detailItem}>
            <Text style={styles.detailLabel}>
              {t("agents.created", "安装时间")}:
            </Text>
            <Text style={styles.detailValue}>
