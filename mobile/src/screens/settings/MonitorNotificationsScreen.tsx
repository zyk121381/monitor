...
const MonitorNotificationsScreen: React.FC = () => {
    ...
    const loadData = async () => {
      try {
        ...
        try {
          ...
          if (configResponse.success && configResponse.data) {
            ...
            if (configResponse.data.settings) {
              ...
              console.log(
                "Found monitors field, using top-level object as settings"
              );
            ...
            if (
              ...
              typeof notificationSettings.monitors !== "object"
            ) {
              console.warn(
                "monitors field is missing or incorrect format, using mock data"
              );
            ...
            if (!Array.isArray(notificationSettings.monitors.channels)) {
              console.warn(
                "monitors.channels is not an array, fixing to empty array"
              );
        ...
        try {
          ...
          console.log(
            "Monitors list response status:",
            monitorsResponse.success
          );

          if (
            monitorsResponse.success &&
            monitorsResponse.monitors &&
            monitorsResponse.monitors.length > 0
          ) {
            console.log(
              "Successfully got monitors list, count:",
              monitorsResponse.monitors.length
            );
            setMonitors(monitorsResponse.monitors);
          } else {
            console.warn(
              "Using mock monitors data:",
              monitorsResponse.message || "No monitors available"
            );
            setMonitors(mockMonitors);
          }
        } catch (monitorsError) {
          console.error(
            "Exception getting monitors list, using mock data",
            monitorsError
          );
          setMonitors(mockMonitors);
        }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.log(
        "Current global monitor settings:",
        JSON.stringify(settings.monitors)
      );
      console.log(
        "Current specific monitor settings count:",
        Object.keys(settings.specificMonitors || {}).length
      );
      ...
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>
            {t("notifications.specificSettings.title", "特定监控通知设置")}
          </Text>
          <TouchableOpacity
            ...
            {monitors.length === 0 ? (
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptyStateText}>
                  {t(
                    "notifications.specificSettings.noMonitors",
                    "没有可用的监控项目"
                  )}
                </Text>
              </View>
                  ...
                  <View style={styles.specificSettingHeader}>
                    ...
                    <View
                      style={[
                        ...
                        {
                          backgroundColor:
                            monitor.status === "up" ? "#4caf50" : "#f44336",
                        },
                      ]}
                    >
                      <Text style={styles.statusText}>
                        {monitor.status === "up"
                          ? t("monitors.status.up", "正常")
                          : t("monitors.status.down", "故障")}
                      </Text>
                    </View>
                  </View>
                    ...
                    <View style={styles.settingItem}>
                      <View style={styles.settingItemContent}>
                        <Text style={styles.settingItemText}>
                          {t("notifications.enabled", "启用通知")}
                        </Text>
                      </View>
                      <Switch
                        value={Boolean(
                          settings.specificMonitors[monitor.id]?.enabled
                        )}
                        onValueChange={(value) =>
                    ...
                    {Boolean(
                      ...
                      <>
                        <View style={styles.settingItem}>
                          ...
                          <Switch
                            value={Boolean(
                              settings.specificMonitors[monitor.id]?.onDown
                            )}
                            onValueChange={(value) =>
                              handleSpecificMonitorSettingChange(
                                monitor.id,
                        ...
                        <View style={styles.settingItem}>
                          ...
                          <Switch
                            value={Boolean(
                              settings.specificMonitors[monitor.id]?.onRecovery
                            )}
                            onValueChange={(value) =>
                              handleSpecificMonitorSettingChange(
                                monitor.id,
                        ...
                        <View style={styles.channelsContainer}>
                          ...
                          {channels.length === 0 ? (
                            <Text style={styles.noChannelsText}>
                              {t(
                                "notifications.channelSettings.noChannels",
                                "没有可用的通知渠道"
                              )}
                            </Text>
                          ) : (
                              ...
                              <View key={channel.id} style={styles.channelItem}>
                                <View style={styles.channelItemContent}>
                                  ...
                                  <Text style={styles.channelItemType}>
                                    (
                                    {t(
                                      `notifications.channelSettings.typeOptions.${channel.type}`,
                                      channel.type
                                    )}
                                    )
                                  </Text>
                                ...
                                <Switch
                                  value={settings.specificMonitors[
                                    monitor.id
                                  ]?.channels?.includes(channel.id)}
                                  onValueChange={(checked) => {
                                    const currentChannels =
                                      settings.specificMonitors[monitor.id]
                                        ?.channels || [];
