...
const GlobalSettingsScreen: React.FC = () => {
    ...
    const loadData = async () => {
      try {
        ...
        if (configResponse.success && configResponse.data) {
          ...
          console.error(
            "Failed to get notification config:",
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.success) {
        Alert.alert(t("common.success"), t("notifications.save.success"), [
          { text: t("common.ok") },
        ]);
      } else {
        Alert.alert(
      ...
      <SafeAreaWrapper>
        <View style={styles.errorContainer}>
          ...
          <Text style={styles.errorText}>
            {error || t("notifications.loadFailed", "加载失败")}
          </Text>
          <TouchableOpacity
      ...
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>
            {t("notifications.settings.title", "通知设置")}
          </Text>
          <TouchableOpacity
        ...
        >
          ...
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>
              {t("notifications.globalSettingsConfig.global", "全局设置")}
            </Text>
            <Text style={styles.sectionDescription}>
              {t(
                "notifications.globalSettingsConfig.globalDescription",
                "配置系统级别的全局通知设置，这些设置将作为默认值应用于所有监控和客户端。"
              )}
            </Text>
            ...
            <View style={styles.infoCard}>
              ...
              <Text style={styles.infoText}>
                {t(
                  "notifications.globalSettingsConfig.globalInfo",
                  "在此页面配置全局默认通知设置，包括监控告警和客户端告警的基本配置。您可以在各自的设置页面中配置特定监控或客户端的通知。"
                )}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>
              {t("settings.notificationsMonitor", "全局监控告警通知")}
            </Text>
            <Text style={styles.sectionDescription}>
              {t(
                "notifications.monitors.description",
                "配置适用于所有API监控的全局默认告警通知设置。"
              )}
            </Text>
            ...
            <View style={styles.settingItem}>
              <View style={styles.settingItemContent}>
                <Text style={styles.settingItemText}>
                  {t("notifications.monitorAlertEnabled", "启用监控告警")}
                </Text>
                <Text style={styles.settingItemDescription}>
                  {t(
                    "notifications.monitors.enabledDescription",
                    "开启后将接收所有监控的告警通知。"
                  )}
                </Text>
              </View>
            ...
            {Boolean(settings.monitors.enabled) && (
              <>
                <View style={styles.settingItem}>
                  <View style={styles.settingItemContent}>
                    <Text style={styles.settingItemText}>
                      {t("notifications.monitors.onDown", "故障时通知")}
                    </Text>
                    <Text style={styles.settingItemDescription}>
                      {t(
                        "notifications.monitors.onDownDescription",
                        "当监控检测到故障时发送通知。"
                      )}
                    </Text>
                  </View>
                ...
                <View style={styles.settingItem}>
                  <View style={styles.settingItemContent}>
                    <Text style={styles.settingItemText}>
                      {t("notifications.monitors.onRecovery", "恢复时通知")}
                    </Text>
                    <Text style={styles.settingItemDescription}>
                      {t(
                        "notifications.monitors.onRecoveryDescription",
                        "当监控从故障状态恢复时发送通知。"
                      )}
                    </Text>
                  </View>
                  <Switch
                    ...
                    trackColor={{ false: "#f0f0f0", true: "#bde0ff" }}
                    thumbColor={
                      settings.monitors.onRecovery ? "#0066cc" : "#ccc"
                    }
                  />
                </View>
                ...
                <View style={styles.channelsSection}>
                  <Text style={styles.channelsSectionTitle}>
                    {t("notifications.specificSettings.channels", "通知渠道")}
                  </Text>
                    ...
                    channels.map((channel) => (
                      <View key={channel.id} style={styles.channelItem}>
                        <View style={styles.channelItemContent}>
                          <Text style={styles.channelItemName}>
                            {channel.name}
                          </Text>
                          <Text style={styles.channelItemType}>
                            (
                            {t(
                        ...
                        <Switch
                          value={settings.monitors.channels?.includes(
                            channel.id
                          )}
                          onValueChange={(checked) => {
                            const updatedChannels = checked
                              ? [
                                  ...(settings.monitors.channels || []),
                                  channel.id,
                                ]
                              : (settings.monitors.channels || []).filter(
                                  (id) => id !== channel.id
                                );
                          ...
                          trackColor={{ false: "#f0f0f0", true: "#bde0ff" }}
                          thumbColor={
                            settings.monitors.channels?.includes(channel.id)
                              ? "#0066cc"
                              : "#ccc"
                          }
                        />
          ...
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>
              {t("settings.notificationsAgent", "全局客户端告警通知")}
            </Text>
            <Text style={styles.sectionDescription}>
              {t(
                "notifications.agents.description",
                "配置适用于所有客户端的全局默认告警通知设置。"
              )}
            </Text>
            ...
            <View style={styles.settingItem}>
              <View style={styles.settingItemContent}>
                <Text style={styles.settingItemText}>
                  {t("notifications.agentAlertEnabled", "启用客户端告警")}
                </Text>
                <Text style={styles.settingItemDescription}>
                  {t(
                    "notifications.agents.enabledDescription",
                    "开启后将接收所有客户端的告警通知。"
                  )}
                </Text>
              </View>
            ...
            {Boolean(settings.agents.enabled) && (
              <>
                <View style={styles.settingItem}>
                  <View style={styles.settingItemContent}>
                    <Text style={styles.settingItemText}>
                      {t("notifications.agents.onOffline", "离线时通知")}
                    </Text>
                    <Text style={styles.settingItemDescription}>
                      {t(
                        "notifications.agents.onOfflineDescription",
                        "当客户端离线时发送通知。"
                      )}
                    </Text>
                  </View>
                ...
                <View style={styles.settingItem}>
                  <View style={styles.settingItemContent}>
                    <Text style={styles.settingItemText}>
                      {t("notifications.agents.onRecovery", "恢复时通知")}
                    </Text>
                    <Text style={styles.settingItemDescription}>
                      {t(
                        "notifications.agents.onRecoveryDescription",
                        "当客户端从离线状态恢复时发送通知。"
                      )}
                    </Text>
                  </View>
                ...
                <View style={styles.settingItem}>
                  <View style={styles.settingItemContent}>
                    <Text style={styles.settingItemText}>
                      {t("notifications.agents.onCpuThreshold", "CPU阈值告警")}
                    </Text>
                    <Text style={styles.settingItemDescription}>
                      {t(
                        "notifications.agents.onCpuThresholdDescription",
                        "当CPU使用率超过阈值时发送通知。"
                      )}
                    </Text>
                  </View>
                  <Switch
                    ...
                    trackColor={{ false: "#f0f0f0", true: "#bde0ff" }}
                    thumbColor={
                      settings.agents.onCpuThreshold ? "#0066cc" : "#ccc"
                    }
                  />
                </View>
                ...
                {Boolean(settings.agents.onCpuThreshold) && (
                  <View style={styles.thresholdContainer}>
                    <Text style={styles.thresholdLabel}>
                      {t("notifications.agents.cpuThreshold", "CPU阈值")}
                    </Text>
                    ...
                    <Text style={styles.thresholdDescription}>
                      {t(
                        "notifications.agents.cpuThresholdDescription",
                        "输入1到100之间的值，当CPU使用率超过此阈值时将触发告警"
                      )}
                    </Text>
                  </View>
                )}
                ...
                <View style={styles.settingItem}>
                  <View style={styles.settingItemContent}>
                    <Text style={styles.settingItemText}>
                      {t(
                        "notifications.agents.onMemoryThreshold",
                        "内存阈值告警"
                      )}
                    </Text>
                    <Text style={styles.settingItemDescription}>
                      {t(
                        "notifications.agents.onMemoryThresholdDescription",
                        "当内存使用率超过阈值时发送通知。"
                      )}
                    </Text>
                  </View>
                  <Switch
                    ...
                    trackColor={{ false: "#f0f0f0", true: "#bde0ff" }}
                    thumbColor={
                      settings.agents.onMemoryThreshold ? "#0066cc" : "#ccc"
                    }
                  />
                </View>
                ...
                {Boolean(settings.agents.onMemoryThreshold) && (
                  <View style={styles.thresholdContainer}>
                    <Text style={styles.thresholdLabel}>
                      {t("notifications.agents.memoryThreshold", "内存阈值")}
                    </Text>
                    <View style={styles.inputContainer}>
                      <TextInput
                        style={styles.thresholdInput}
                    ...
                    <Text style={styles.thresholdDescription}>
                      {t(
                        "notifications.agents.memoryThresholdDescription",
                        "输入1到100之间的值，当内存使用率超过此阈值时将触发告警"
                      )}
                    </Text>
                  </View>
                )}
                ...
                <View style={styles.settingItem}>
                  <View style={styles.settingItemContent}>
                    <Text style={styles.settingItemText}>
                      {t(
                        "notifications.agents.onDiskThreshold",
                        "磁盘阈值告警"
                      )}
                    </Text>
                    <Text style={styles.settingItemDescription}>
                      {t(
                        "notifications.agents.onDiskThresholdDescription",
                        "当磁盘使用率超过阈值时发送通知。"
                      )}
                    </Text>
                  </View>
                  <Switch
                    ...
                    trackColor={{ false: "#f0f0f0", true: "#bde0ff" }}
                    thumbColor={
                      settings.agents.onDiskThreshold ? "#0066cc" : "#ccc"
                    }
                  />
                </View>
                ...
                {Boolean(settings.agents.onDiskThreshold) && (
                  <View style={styles.thresholdContainer}>
                    <Text style={styles.thresholdLabel}>
                      {t("notifications.agents.diskThreshold", "磁盘阈值")}
                    </Text>
                    <View style={styles.inputContainer}>
                      <TextInput
                        style={styles.thresholdInput}
                    ...
                    <Text style={styles.thresholdDescription}>
                      {t(
                        "notifications.agents.diskThresholdDescription",
                        "输入1到100之间的值，当磁盘使用率超过此阈值时将触发告警"
                      )}
                    </Text>
                  </View>
                )}
                      ...
                      <View key={channel.id} style={styles.channelItem}>
                        ...
                        <Switch
                          ...
                          onValueChange={(checked) => {
                            const updatedChannels = checked
                              ? [
                                  ...(settings.agents.channels || []),
                                  channel.id,
                                ]
                              : (settings.agents.channels || []).filter(
                                  (id) => id !== channel.id
                                );
                          ...
                          trackColor={{ false: "#f0f0f0", true: "#bde0ff" }}
                          thumbColor={
                            settings.agents.channels?.includes(channel.id)
                              ? "#0066cc"
                              : "#ccc"
                          }
                        />
          ...
          >
            <View style={styles.navigationCardContent}>
              ...
              <View style={styles.navigationCardTextContainer}>
                <Text style={styles.navigationCardTitle}>
                  {t("notifications.monitorSettings.title", "监控项目通知")}
                </Text>
                <Text style={styles.navigationCardDescription}>
                  {t(
                    "notifications.monitorSettings.description",
                    "配置具体监控项目的通知规则"
                  )}
                </Text>
              </View>
          ...
          {/* 代理通知设置 */}
          <View style={styles.section}>{/* ... existing code ... */}</View>
          ...
          >
            <View style={styles.navigationCardContent}>
              ...
              <View style={styles.navigationCardTextContainer}>
                <Text style={styles.navigationCardTitle}>
                  {t("notifications.agentSettings.title", "代理通知设置")}
                </Text>
                <Text style={styles.navigationCardDescription}>
                  {t(
                    "notifications.agentSettings.description",
                    "配置具体代理的通知规则"
                  )}
                </Text>
              </View>
          ...
          >
            <View style={styles.navigationCardContent}>
              ...
              <View style={styles.navigationCardTextContainer}>
                <Text style={styles.navigationCardTitle}>
                  {t("notifications.channelSettings.title", "通知渠道")}
                </Text>
                <Text style={styles.navigationCardDescription}>
                  {t(
                    "notifications.channelSettings.description",
                    "管理不同的通知渠道和配置"
                  )}
                </Text>
              </View>
