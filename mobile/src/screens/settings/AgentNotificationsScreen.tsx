...
const AgentNotificationsScreen: React.FC = () => {
  ...
  const localizedMockChannels = useMemo(
    () => [
      {
        id: "1",
        name: t("notifications.channelSettings.typeOptions.app", "应用内"),
        type: "app",
        enabled: true,
        config: {},
      },
      {
        id: "2",
        name: t("notifications.channelSettings.typeOptions.email", "邮件"),
        type: "email",
        enabled: true,
      ...
      {
        id: "3",
        name: t("notifications.channelSettings.typeOptions.wechat", "微信"),
        type: "wechat",
        enabled: true,
        config: {},
      },
    ...
    const loadData = async () => {
      try {
        ...
        try {
          ...
          console.log(
            "Notification config response status:",
            configResponse.success
          );

          if (configResponse.success && configResponse.data) {
            console.log(
              "Notification config response data format:",
              typeof configResponse.data
            );
            ...
            if (configResponse.data.settings) {
              ...
              console.log(
                "Found agents field, using top-level object as settings"
              );
              ...
              console.warn(
                "Unrecognized data structure, will try to use the entire data object"
              );
              notificationSettings = configResponse.data;
            ...
            console.log(
              "Processed settings object contains fields:",
              Object.keys(notificationSettings || {}).join(", ")
            );
            ...
            if (
              ...
              typeof notificationSettings.agents !== "object"
            ) {
              console.warn(
                "agents field is missing or incorrect format, using mock data"
              );
            ...
            if (!notificationSettings.specificAgents) {
              console.warn(
                "specificAgents field is missing, using empty object"
              );
            ...
            if (!notificationSettings.specificMonitors) {
              console.warn(
                "specificMonitors field is missing, using empty object"
              );
            ...
            if (!Array.isArray(notificationSettings.agents.channels)) {
              console.warn(
                "agents.channels is not an array, fixing to empty array"
              );
            ...
            setChannels(
              channelList.length > 0 ? channelList : localizedMockChannels
            );
          } else {
            console.warn(
              "Failed to get notification config, using mock data",
          ...
          console.error(
            "Exception getting notification config, using mock data",
            configError
          );
          setSettings(mockSettings);
          setChannels(localizedMockChannels);
        ...
        try {
          ...
          if (
            ...
            console.log(
              "Successfully got agents list, count:",
              agentsResponse.agents.length
            );
            setAgents(agentsResponse.agents);
          } else {
            console.warn(
              "Using mock agents data:",
              agentsResponse.message || "No agents available"
            );
            setAgents(localizedMockAgents);
          }
        } catch (agentsError) {
          console.error(
            "Exception getting agents list, using mock data",
            agentsError
          );
          setAgents(localizedMockAgents);
        }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.log(
        "Current global agent settings:",
        JSON.stringify(settings.agents)
      );
      console.log(
        "Current specific agent settings count:",
        Object.keys(settings.specificAgents || {}).length
      );
      ...
      if (response.success) {
        ...
        Alert.alert(t("common.success"), t("notifications.save.success"), [
          {
            text: t("common.ok"),
            onPress: () => console.log("Save success dialog closed"),
          },
        ]);
      } else {
        ...
        Alert.alert(
          ...
          [
            {
              text: t("common.ok"),
              onPress: () => console.log("Save failure dialog closed"),
            },
          ]
        );
      }
      ...
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>
            {t(
              "notifications.specificAgentSettings.title",
              "特定客户端通知设置"
            )}
          </Text>
          <TouchableOpacity
        ...
        >
          ...
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>
              {t(
                "notifications.specificSettings.title",
                "Specific Client Notification Settings"
              )}
            </Text>
            <Text style={styles.sectionDescription}>
              {t(
                "notifications.specificSettings.description",
                "Configure separate notification settings for each specific client."
              )}
            </Text>
            ...
            {agents.length === 0 ? (
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptyStateText}>
                  {t(
                    "notifications.specificSettings.noAgents",
                    "No agents available"
                  )}
                </Text>
              </View>
                ...
                <View key={agent.id} style={styles.specificSettingItem}>
                  <View style={styles.specificSettingHeader}>
                    <View style={styles.specificSettingTitleContainer}>
                      <Text style={styles.specificSettingTitle}>
                        {agent.name}
                      </Text>
                    ...
                    <View
                      style={[
                        ...
                        {
                          backgroundColor:
                            agent.status === "active" ? "#4caf50" : "#f44336",
                        },
                      ]}
                    >
                      <Text style={styles.statusText}>
                        {agent.status === "active"
                          ? t("agents.status.active", "Online")
                          : t("agents.status.inactive", "Offline")}
                      </Text>
                    </View>
                  </View>
                  ...
                  <View style={styles.specificSettings}>
                    <View style={styles.settingItem}>
                      ...
                      <Switch
                        value={Boolean(
                          settings.specificAgents[agent.id]?.enabled
                        )}
                        onValueChange={(value) =>
                    ...
                    {Boolean(settings.specificAgents[agent.id]?.enabled) && (
                      <>
                          ...
                          <View style={styles.settingItemContent}>
                            <Text style={styles.settingItemText}>
                              {t(
                                ...
                                "Notify when client is offline"
                              )}
                            </Text>
                          </View>
                          <Switch
                            value={Boolean(
                              settings.specificAgents[agent.id]?.onOffline
                            )}
                            onValueChange={(value) =>
                              handleSpecificAgentSettingChange(
                                agent.id,
                          ...
                          <View style={styles.settingItemContent}>
                            <Text style={styles.settingItemText}>
                              {t(
                                ...
                                "Notify when client recovers online"
                              )}
                            </Text>
                          </View>
                          <Switch
                            value={Boolean(
                              settings.specificAgents[agent.id]?.onRecovery
                            )}
                            onValueChange={(value) =>
                              handleSpecificAgentSettingChange(
                                agent.id,
                        ...
                        <View style={styles.settingItem}>
                          ...
                          <Switch
                            value={Boolean(
                              settings.specificAgents[agent.id]?.onCpuThreshold
                            )}
                            onValueChange={(value) =>
                              handleSpecificAgentSettingChange(
                                agent.id,
                        ...
                        {Boolean(
                          ...
                          <View style={styles.thresholdContainer}>
                            <Text style={styles.thresholdLabel}>
                              {t("notifications.threshold.label", "Threshold")}
                            </Text>
                            <TextInput
                              style={styles.thresholdInput}
                              value={String(
                                settings.specificAgents[agent.id]
                                  ?.cpuThreshold || ""
                              )}
                              onChangeText={(text) =>
                                handleSpecificAgentSettingChange(
                        ...
                        {Boolean(
                          settings.specificAgents[agent.id]?.onMemoryThreshold
                        ) && (
                          <View style={styles.thresholdContainer}>
                            <Text style={styles.thresholdLabel}>
                              {t("notifications.threshold.label", "Threshold")}
                        ...
                        <View style={styles.settingItem}>
                          ...
                          <Switch
                            value={Boolean(
                              settings.specificAgents[agent.id]?.onDiskThreshold
                            )}
                            onValueChange={(value) =>
                              handleSpecificAgentSettingChange(
                                agent.id,
                        ...
                        <View style={styles.channelsContainer}>
                          <Text style={styles.channelsTitle}>
                            {t(
                              "notifications.specificSettings.channels",
