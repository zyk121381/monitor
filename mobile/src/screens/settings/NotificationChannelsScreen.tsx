        ...
        if (
          ...
          setError(
            configResponse.message ||
              t("notifications.channelSettings.loadFailed")
          );
        }
      } catch (error) {
  ...
  const handleSaveChannel = async () => {
    try {
      if (!channelForm.name.trim()) {
        Alert.alert(
          t("common.error"),
          t("notifications.channelSettings.nameRequired")
        );
        return;
      }
      ...
      if (channelForm.type === "email" && !channelForm.config.email) {
        Alert.alert(
          t("common.error"),
          t("notifications.channelSettings.emailRequired")
        );
        return;
      }
      ...
      if (channelForm.type === "webhook" && !channelForm.config.webhook_url) {
        Alert.alert(
          t("common.error"),
          t("notifications.channelSettings.webhookUrlRequired")
        );
        return;
      }
      ...
      if (
        ...
        Alert.alert(
          t("common.error"),
          t("notifications.channelSettings.tokenAndChatIdRequired")
        );
        return;
      }
      ...
      if (isAddModalVisible) {
        ...
        if (response.success) {
          Alert.alert(
            t("common.success"),
            t("notifications.channelSettings.addSuccess")
          );
          ...
          Alert.alert(
            t("common.error"),
            response.message || t("notifications.channelSettings.addFailed")
          );
        }
        ...
        if (response.success) {
          Alert.alert(
            t("common.success"),
            t("notifications.channelSettings.updateSuccess")
          );
          ...
          Alert.alert(
            t("common.error"),
            response.message || t("notifications.channelSettings.updateFailed")
          );
        }
      }
    } catch (error) {
      ...
      Alert.alert(
        t("common.error"),
        t("notifications.channelSettings.saveFailed")
      );
    }
  };
          ...
          onPress: async () => {
            try {
              ...
              if (response.success) {
                Alert.alert(
                  t("common.success"),
                  t("notifications.channelSettings.deleteSuccess")
                );
                ...
                Alert.alert(
                  t("common.error"),
                  response.message ||
                    t("notifications.channelSettings.deleteFailed")
                );
              }
            } catch (error) {
            ...
            {channels.length === 0 ? (
              <View style={styles.emptyContainer}>
                ...
                <Text style={styles.emptySubText}>
                  {t(
                    "notifications.channelSettings.addPrompt",
                    "点击上方按钮添加一个渠道"
                  )}
                </Text>
              </View>
                        ...
                        onValueChange={(enabled) => {
                          ...
                          updateNotificationChannel(channel.id, updatedChannel)
                            .then((response) => {
                              if (response.success) {
                                ...
                                Alert.alert(
                                  ...
                                  response.message ||
                                    t(
                                      "notifications.channelSettings.updateFailed"
                                    )
                                );
                              }
                            })
                            .catch((error) => {
                              ...
                              Alert.alert(
                                t("common.error"),
                                t("notifications.channelSettings.updateFailed")
                              );
                            });
                        }}
                  ...
                  <View style={styles.channelDetails}>
                    {channel.type === "email" && channel.config.email && (
                      <Text style={styles.channelConfigText}>
                        <Text style={styles.channelConfigLabel}>
                          {t("notifications.channelSettings.email")}:
                        </Text>{" "}
                    ...
                    {channel.type === "webhook" &&
                      channel.config.webhook_url && (
                        <Text style={styles.channelConfigText}>
                          <Text style={styles.channelConfigLabel}>
                            {t("notifications.channelSettings.webhookUrl")}:
                          </Text>{" "}
                          {channel.config.webhook_url}
                        </Text>
                      )}
              ...
              <ScrollView style={styles.modalContent}>
                ...
                <View style={styles.formGroup}>
                  <Text style={styles.formLabel}>
                    {t("notifications.channelSettings.name", "渠道名称")}
                  </Text>
                  <TextInput
                    style={styles.formInput}
                    value={channelForm.name}
                    onChangeText={(text) =>
                      setChannelForm({ ...channelForm, name: text })
                    }
                    placeholder={t(
                      "notifications.channelSettings.namePlaceholder",
                      "请输入渠道名称"
                    )}
                  />
                </View>
                ...
                <View style={styles.formGroup}>
                  <Text style={styles.formLabel}>
                    {t("notifications.channelSettings.channelType", "渠道类型")}
                  </Text>
                  <View style={styles.typeSelector}>
                    {channelTypes.map((type) => (
                      <TouchableOpacity
                        ...
                        style={[
                          styles.typeButton,
                          channelForm.type === type.id &&
                            styles.typeButtonSelected,
                        ]}
                        onPress={() =>
                          setChannelForm({ ...channelForm, type: type.id })
                        }
                      >
                        <Text
                          style={[
                        ...
                        >
                          {t(
                            `notifications.channelSettings.typeOptions.${type.id}`,
                            type.name
                          )}
                        </Text>
                      </TouchableOpacity>
                ...
                {channelForm.type === "email" && (
                  <View style={styles.formGroup}>
                    <Text style={styles.formLabel}>
                      {t("notifications.channelSettings.email", "邮箱地址")}
                    </Text>
                    <TextInput
                      style={styles.formInput}
                ...
                {channelForm.type === "webhook" && (
                  <View style={styles.formGroup}>
                    <Text style={styles.formLabel}>
                      {t(
                        "notifications.channelSettings.webhookUrl",
                        "Webhook URL"
                      )}
                    </Text>
                    <TextInput
                ...
                {(channelForm.type === "telegram" ||
                  ...
                  <>
                    <View style={styles.formGroup}>
                      <Text style={styles.formLabel}>
                        {t("notifications.channelSettings.token", "Token")}
                      </Text>
                      <TextInput
                        style={styles.formInput}
                    ...
                    <View style={styles.formGroup}>
                      <Text style={styles.formLabel}>
                        {t("notifications.channelSettings.chatId", "Chat ID")}
                      </Text>
                      <TextInput
                        style={styles.formInput}
                ...
                <View style={styles.formGroup}>
                  <View style={styles.enabledContainer}>
                    <Text style={styles.formLabel}>
                      {t("notifications.channelSettings.enabled", "启用状态")}
                    </Text>
                    <Switch
