...
const SimpleHeartbeatChart: React.FC<{
  ...
  // 根据容器宽度计算可以容纳的格子数量
  const calculatedItemCount = Math.floor(
    (contentWidth + barGap) / (barWidth + barGap)
  );
  ...
  const adjustedBarWidth =
    (contentWidth - barGap * (fixedItemCount - 1)) / fixedItemCount;
...
// 路由参数类型
type MonitorDetailRouteProp = RouteProp<
  { MonitorDetail: { monitorId: string } },
  "MonitorDetail"
>;
...
const MonitorDetailScreen: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // 获取监控历史
      const historyData = await monitorService.getMonitorHistory(
        Number(monitorId)
      );

      // 转换历史数据格式以兼容界面显示
      const formattedHistory: MonitorStatusHistory[] = historyData.map(
        (item: any) => ({
      ...
      Alert.alert(
        t("common.error", "错误"),
        t("monitors.fetchDetailFailed", "获取监控详情失败")
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleCheckNow = async () => {
    ...
    try {
      ...
      await fetchData();
      Alert.alert(
        t("monitors.checkNowSuccess", "检查成功"),
        t(
          "monitors.checkNowSuccessMessage",
          "已成功发起检查，请稍后刷新查看结果"
        )
      );
    } catch (error) {
      ...
      Alert.alert(
        t("common.error", "错误"),
        t("monitors.checkNowFailed", "立即检查失败")
      );
    } finally {
          ...
          onPress: async () => {
            try {
              ...
              if (result.success) {
                ...
                Alert.alert(
                  t("common.error", "错误"),
                  result.message || t("monitors.deleteFailed", "删除监控失败")
                );
              }
            } catch (error) {
              ...
              Alert.alert(
                t("common.error", "错误"),
                t("monitors.deleteFailed", "删除监控失败")
              );
            }
          },
        },
    ...
    <SafeAreaWrapper>
      ...
      >
        ...
        <View style={styles.card}>
          ...
          <View style={styles.detailItem}>
            <Text style={styles.detailLabel}>
              {t("monitors.method", "方法")}:
            </Text>
            <Text style={styles.detailValue}>{monitor.method}</Text>
          </View>
          <View style={styles.detailItem}>
            <Text style={styles.detailLabel}>
              {t("monitors.interval", "检查间隔")}:
            </Text>
            <Text style={styles.detailValue}>
              {formatDuration(monitor.interval)}
            </Text>
          </View>
          <View style={styles.detailItem}>
            <Text style={styles.detailLabel}>
              {t("monitors.timeout", "超时")}:
            </Text>
            <Text style={styles.detailValue}>{monitor.timeout}s</Text>
          </View>
          <View style={styles.detailItem}>
            <Text style={styles.detailLabel}>
              {t("monitors.created", "创建时间")}:
            </Text>
            <Text style={styles.detailValue}>
        ...
        <View style={styles.card}>
          <Text style={styles.cardTitle}>
            {t("monitors.statusDetails.title", "当前状态")}
          </Text>
          <View style={styles.statusContainer}>
            <View style={styles.statusItem}>
              <Text style={styles.statusLabel}>
                {t("monitors.statusDetails.lastCheck", "最后检查")}
              </Text>
              <Text style={styles.statusValue}>
                {formatDate(monitor.last_check || monitor.last_checked)}
              </Text>
            </View>
            <View style={styles.statusItem}>
              <Text style={styles.statusLabel}>
                {t("monitors.statusDetails.uptime", "在线率")}
              </Text>
              <Text style={styles.statusValue}>
            ...
            <View style={styles.statusItem}>
              <Text style={styles.statusLabel}>
                {t("monitors.statusDetails.responseTime", "响应时间")}
              </Text>
              <Text style={styles.statusValue}>{monitor.response_time}ms</Text>
            </View>
            <View style={styles.statusItem}>
              <Text style={styles.statusLabel}>
                {t("monitors.statusDetails.active", "活动状态")}
              </Text>
              <Text
                style={[
                  styles.statusValue,
        ...
        {history.length > 0 && (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>
              {t("monitors.charts.statusHistory", "状态历史")}
            </Text>
        ...
        <View style={styles.card}>
          <Text style={styles.cardTitle}>
            {t("monitors.historyRecords.title", "历史记录")}
          </Text>
          <View style={styles.historyList}>
            {history.length === 0 ? (
              <Text style={styles.emptyText}>
                {t("monitors.historyRecords.empty", "暂无历史记录")}
              </Text>
            ) : (
            ...
            {history.length > 5 && (
              <TouchableOpacity
                ...
                onPress={() =>
                  navigation.navigate("MonitorHistory", { monitorId })
                }
              >
