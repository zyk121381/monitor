...
export async function generateToken(env?: any): Promise<string> {
  ...
  console.log("generateToken 版本信息:", { tag: versionTag, timestamp: versionTimestamp });
...
export async function verifyToken(
  token: string, 
  env: any, 
): Promise<{ valid: boolean; message?: string; timestamp?: number; payload?: any }> {
  console.log('=== 开始验证令牌 ===');
  ...
  try {
    ...
    console.log('解析的时间戳 (Base36 -> 数字):', timestampStr, '->', timestamp);
    ...
    console.log("verifyToken 版本信息:", { tag: versionTag, timestamp: versionTimestamp });
    ...
    const hashBuffer = await crypto.subtle.digest('SHA-256', msgUint8);
    const hashArray = Array.from(new Uint8Array(hashBuffer));
    const calculatedSignature = hashArray.map(b => b.toString(16).padStart(2, '0')).join('').substring(0, 16);
