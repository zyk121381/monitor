...
export async function loginUser(
  ...
  try {
    ...
    console.log('传入的 env 参数结构:', JSON.stringify(env, (key, value) => {
      // 排除可能的循环引用对象
    ...
    if (env.CF_VERSION_METADATA) {
      ...
      console.log('CF_VERSION_METADATA 内容:', JSON.stringify(env.CF_VERSION_METADATA, null, 2));
    } else {
    ...
    try {
      ...
      console.error('JWT 错误堆栈:', jwtError instanceof Error ? jwtError.stack : '未知错误');
