...
export const deleteNotificationChannel = async (
  ...
  // 先删除通知历史记录表中的关联记录
  await db
    .prepare("DELETE FROM notification_history WHERE channel_id = ?")
    .bind(id)
    .run();

  // 再检查并更新通知设置中的channels列表
  const allSettings = await db
    .prepare("SELECT id, channels FROM notification_settings")
      ...
      try {
        ...
        if (JSON.stringify(channelsList) !== JSON.stringify(newChannelsList)) {
          await db
            ...
            .bind(JSON.stringify(newChannelsList), setting.id)
            .run();
        }
      } catch (error) {
...
export const getGlobalSettings = async (
  ...
  const monitorSettings = await db
    .prepare("SELECT * FROM notification_settings WHERE target_type = ?")
    .bind("global-monitor")
...
export const createOrUpdateSettings = async (
  ...
  const existingSettings = await db
    ...
    .bind(settings.target_type, settings.target_id, settings.target_id)
    .first<{ id: number }>();
  ...
  if (existingSettings) {
    ...
    const result = await db
      .prepare(
        `INSERT INTO notification_settings (${keys.join(
          ", "
