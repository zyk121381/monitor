...
// 添加示例监控数据
async function addSampleMonitors(c: any) {
  // 检查是否已有示例监控数据
  ...
  if (existingMonitors.count === 0) {
    ...
    // 添加API服务监控
    const apiMonitor = await c.env.DB.prepare(
    ...
    // 添加用户服务监控
    const userMonitor = await c.env.DB.prepare(
      `INSERT INTO monitors (name, url, method, interval, timeout, expected_status, headers, body, created_by, active, status, uptime, response_time, last_checked, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).bind(
      '用户服务',
      'https://users.example.com/health',
      'GET',
      30,
      30,
      200,
    ...
    // 添加支付服务监控
    const paymentMonitor = await c.env.DB.prepare(
      `INSERT INTO monitors (name, url, method, interval, timeout, expected_status, headers, body, created_by, active, status, uptime, response_time, last_checked, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).bind(
      '支付服务',
    ...
    // 添加数据库服务监控
    const dbMonitor = await c.env.DB.prepare(
      `INSERT INTO monitors (name, url, method, interval, timeout, expected_status, headers, body, created_by, active, status, uptime, response_time, last_checked, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).bind(
      '数据库服务',
...
// 添加示例客户端数据
async function addSampleAgents(c: any) {
  // 检查是否已有示例客户端数据
  ...
  if (existingAgents.count === 0) {
    ...
    // 主服务器
    const primaryAgent = await c.env.DB.prepare(
    ...
    // 备份服务器
    const backupAgent = await c.env.DB.prepare(
      `INSERT INTO agents (name, token, created_by, status, created_at, updated_at, cpu_usage, memory_total, memory_used, disk_total, disk_used, network_rx, network_tx, hostname, ip_address, os, version)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).bind(
      '备份服务器',
      'backup-server-token-123456',
      userId,
      'active',
      now,
      now,
    ...
    // API服务器
    const apiAgent = await c.env.DB.prepare(
      `INSERT INTO agents (name, token, created_by, status, created_at, updated_at, cpu_usage, memory_total, memory_used, disk_total, disk_used, network_rx, network_tx, hostname, ip_address, os, version)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).bind(
      'API服务器',
      'api-server-token-123456',
      userId,
      'active',
      now,
      now,
