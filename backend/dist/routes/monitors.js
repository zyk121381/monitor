    ...
    try {
        ...
        if (result.results && result.results.length > 0) {
            const monitorsWithHistory = await Promise.all(result.results.map(async (monitor) => {
                const historyResult = await c.env.DB.prepare(`SELECT * FROM monitor_status_history 
           WHERE monitor_id = ? 
           ORDER BY timestamp ASC`).bind(monitor.id).all();
                return {
                    ...monitor,
...
monitors.get('/:id', async (c) => {
    try {
        ...
        const historyResult = await c.env.DB.prepare(`SELECT * FROM monitor_status_history 
       WHERE monitor_id = ? 
       ORDER BY timestamp ASC`).bind(id).all();
        // 获取最近的检查历史记录
        const checksResult = await c.env.DB.prepare(`SELECT * FROM monitor_checks 
       WHERE monitor_id = ? 
...
monitors.delete('/:id', async (c) => {
    try {
        ...
        // 先删除关联的历史数据
        await c.env.DB.prepare('DELETE FROM monitor_status_history WHERE monitor_id = ?').bind(id).run();
        await c.env.DB.prepare('DELETE FROM monitor_checks WHERE monitor_id = ?').bind(id).run();
        // 执行删除
...
monitors.post('/:id/check', async (c) => {
    try {
      ...
      WHERE id = ?
    `).bind(status, responseTime, now, monitor.id).run();
        // 添加检查记录
        await c.env.DB.prepare(`
